...
import org.mate.util.TimeoutRun;
...
import static org.mate.io.Device.STORE_COVERAGE_DATA_TIMEOUT;
    ...
    public static Message storeCoverageData(AndroidEnvironment androidEnvironment, String deviceID, String packageName,
        ...
        TimeoutRun.timeoutRun(() -> {
            try {
                ...
                File traceFile = device.pullTraceFile(chromosome, entity);
                success[0] = traceFile != null;
            } catch (Exception e) {
                Log.printError("An error occurred while storing coverage data: " + e.getMessage());
            }

            return null;
        }, STORE_COVERAGE_DATA_TIMEOUT);

        if (!success[0]) {
            // An error occurred while storing coverage data.
            // Create an empty dummy traces file for this test case.
            device.createEmptyTraceFile(chromosome, entity);
        }
